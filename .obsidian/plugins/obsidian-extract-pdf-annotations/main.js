...
var ANNOTS_TREATED_AS_HIGHLIGHTS = ["Highlight", "Underline", "Squiggly"];
...
var PDFAnnotationPluginSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Template for notes of PDFs inside Obsidian:").addTextArea(
      (input) => {
        input.inputEl.style.width = "100%";
        input.inputEl.style.height = "10em";
...
function searchQuad(minx, maxx, miny, maxy, items) {
  const mycontent = items.reduce(function(txt, x) {
    ...
    if (x.transform[4] > maxx)
      return txt;
    const start = x.transform[4] >= minx ? 0 : (
      // start at pos 0, when text starts after hightlight start
      Math.round(x.str.length * (minx - x.transform[4]) / x.width)
    );
...
function loadPDFFile(file, pdfjsLib, containingFolder, total, desiredAnnotations) {
  return __async(this, null, function* () {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      yield loadPage(page, i, file, containingFolder, total, desiredAnnotations);
    }
  });
}
...
`;
  ...
  loadSinglePDFFile(file) {
    return __async(this, null, function* () {
      ...
      yield loadPDFFile(PDFFile.convertTFileToPDFFile(file, content), pdfjsLib, containingFolder, grandtotal, desiredAnnotations);
  ...
  loadAnnotationsFromSinglePDFFileFromClipboardPath(filePathFromClipboard) {
    return __async(this, null, function* () {
      ...
      try {
        ...
        if (stats.isFile()) {
          const pdfjsLib = yield (0, import_obsidian2.loadPdfJs)();
          const binaryContent = yield import_obsidian2.FileSystemAdapter.readLocalFile(filePathWithoutQuotes);
          ...
          const file = new PDFFile(fileName, binaryContent, extension, encodedFilePath);
          const containingFolder = filePathWithSlashs.slice(0, filePathWithSlashs.lastIndexOf("/"));
          const desiredAnnotations = this.settings.parsedSettings.desiredAnnotations;
          yield loadPDFFile(file, pdfjsLib, containingFolder, grandtotal, desiredAnnotations);
        } else {
    ...
    return __async(this, null, function* () {
      ...
      this.addCommand({
        ...
        editorCallback: (editor, view) => __async(this, null, function* () {
          ...
          const grandtotal = yield this.loadAnnotationsFromSinglePDFFileFromClipboardPath(clipText);
          this.sort(grandtotal);
          ...
          import_obsidian2.Vault.recurseChildren(folder, (file2) => __async(this, null, function* () {
            if (file2 instanceof import_obsidian2.TFile) {
              if (file2.extension === "pdf") {
                promises.push(
                  this.app.vault.readBinary(file2).then((content) => loadPDFFile(PDFFile.convertTFileToPDFFile(file2, content), pdfjsLib, file2.parent.name, grandtotal, desiredAnnotations))
                );
              }
            }
          }));
